...
AddEventHandler('ra1derPaycheck:menu:Open_menu', function()
     QBCore.Functions.TriggerCallback('ra1derPaycheck:server:account_information', function(result)
          if result then
               Open_menu(result)
               return
          end
...
local function withdraw_all(maximum)
     ...
     QBCore.Functions.TriggerCallback('ra1derPaycheck:server:withdraw_all', function(result, reason)
          if result then
               Speach('thanks')
...
local function withdraw_amount(maximum)
     ...
     if inputData then
          ...
          QBCore.Functions.TriggerCallback('ra1derPaycheck:server:withdraw_amount', function(result, reason)
               if result then
                    TriggerEvent('animations:client:EmoteCommandStart', { "ID" })
                    QBCore.Functions.Notify(Lang:t('success.successful_withdraw') .. reason .. '$', "success")
                    Speach('thanks')
...
function menu:withdraw_menu(data)
     ...
     elseif req == 3 then
          ...
          QBCore.Functions.TriggerCallback('ra1derPaycheck:server:get_logs', function(result)
...
function QbMenu:withdraw_menu(data)
     ...
     local Menu = {
          ...
          },
          {
               ...
               icon = 'fa-solid fa-money-bill-transfer',
               params = {
                    event = 'ra1derPaycheck:client:function_caller',
          ...
          },
          {
               ...
               params = {
                    event = "ra1derPaycheck:client:close_menu",
               }
          }
     }
...
AddEventHandler('ra1derPaycheck:client:function_caller', function(data)
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
     if data.id == 1 then
          -- Withdraw All
          withdraw_all(data.money)
...
AddEventHandler('ra1derPaycheck:client:close_menu', function()
     TriggerEvent('qb-menu:closeMenu')
end)

AddEventHandler('qb-menu:closeMenu', function()
     if not GetCurrentResourceName() == 'ra1derPaycheck' then
          return
     end
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
end)
...
local function spawn_ped(data)
     ...
     -- local ped = exports["elixir-base"]:pedcreate("paycheck", data.model, data.coords.x, data.coords.y, data.coords.z -0.4,  165.5)
...
local function makeCore()
     ...
     Citizen.CreateThread(function()
          ...
          exports['qb-target']:AddBoxZone("ra1der_paycheck", vec3_coord, Config.intraction.box.l, Config.intraction.box.w,
               {
                    name = "ra1der_paycheck",
               ...
               options = {
                    {
                         event = "ra1derPaycheck:menu:Open_menu",
                         icon = "fa-solid fa-credit-card",
