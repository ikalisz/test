...
local function logger(citizenid, state, amount, metadata)
     local query = 'INSERT INTO ra1derPaycheck_logs (citizenid, state, amount, metadata) VALUES (:citizenid, :state, :amount, :metadata)'
     local data = {
          ['citizenid'] = citizenid,
          ['state'] = state,
          ['amount'] = amount,
...
local function init_account(citizenid, amount)
     local query = 'INSERT INTO ra1derPaycheck_account (citizenid, money) VALUES (:citizenid, :money) ON DUPLICATE KEY UPDATE money = :money'
     local data = {
          ['citizenid'] = citizenid,
          ['money'] = amount,
     }
...
local function update_money(citizenid, amount)
     MySQL.Async.execute('UPDATE `ra1derPaycheck_account` SET `money` = ? WHERE `citizenid` = ?',
...
local function addmoney_to_paycheck(citizenid, amount, job)
     ...
     MySQL.Async.fetchAll('SELECT * FROM ra1derPaycheck_account WHERE citizenid = ?', { citizenid }, function(res)
          local m = 0
...
RegisterNetEvent('ra1derPaycheck:server:AddMoneyToPayCheck', function(citizenid, amount, from)
     addmoney_to_paycheck(citizenid, amount, from)
end)

QBCore.Functions.CreateCallback('ra1derPaycheck:server:account_information', function(source, cb)
...
QBCore.Functions.CreateCallback('ra1derPaycheck:server:withdraw_all', function(source, cb)
     local Player = QBCore.Functions.GetPlayer(source)
     local citizenid = Player.PlayerData.citizenid
...
QBCore.Functions.CreateCallback('ra1derPaycheck:server:withdraw_amount', function(source, cb, amount)
     local Player = QBCore.Functions.GetPlayer(source)
     local citizenid = Player.PlayerData.citizenid
...
QBCore.Functions.CreateCallback('ra1derPaycheck:server:get_logs', function(source, cb, limit)
     limit = limit or 10 --placeholder
     local Player = QBCore.Functions.GetPlayer(source)
     local citizenid = Player.PlayerData.citizenid
     local query = 'SELECT state, amount, metadata,created FROM ra1derPaycheck_logs WHERE citizenid = ? order by created desc limit 15'
