...
RegisterNetEvent("ems:medic", function()
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) and spam then
		QBCore.Functions.TriggerCallback('hhfw:docOnline', function(EMSOnline)
			QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty)
				if qtty >= 1 then
					if EMSOnline <= Config.Doctor and spam then
						SpawnVehicle(GetEntityCoords(PlayerPedId()))
						Notify("Vehicle on the Road.", "error")
					else
						if EMSOnline > Config.Doctor then
							Notify("There is already a doctor in the city..", "error")
						else
							Notify("wait already paramedic is on the way!", "primary")
						end	
					end
				else
					Notify("You can't make calls because you don't have a phone!", "primary")
				end
			end, "white_phone")
		end)
	else
		Notify("You are not seriously injured?", "error")
	end
end)
...
function DoctorNPC()
	...
	QBCore.Functions.Progressbar("revive_doc", "The doctor is treating you.", 15000, false, false, {
		...
		ClearPedTasks(test1)
		Citizen.Wait(500)
		TriggerEvent('esx_ambulancejob:revive', PlayerPedId(), true)
		StopScreenEffect('DeathFailOut')	
		TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'ambulance', "Treatment Fee", 2500)
		QBCore.Functions.Notify("Invoice has been issued to the patient")
		Notify("Your treatment was done and it was reflected as a $2500 bill.", "success")
		Wait(15000)
