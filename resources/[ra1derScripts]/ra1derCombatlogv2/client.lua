        ...
        while sure do
            ...
            if dist < 20.0 then
                ...
                if dist < 3 then
                    ...
                    DrawText3Ds(location.x, location.y, location.z + 0.7, "~g~[E] ~w~" ..discord.." ~y~~w~] copy for report | Reason:" ..reason)
...
RegisterCommand("combatlog", function(_, args)
    if not combatlog then
        combatlog = true
        QBCore.Functions.Notify("You can now see debuting players", "success")
    else
        combatlog = false
        QBCore.Functions.Notify("You will no longer see debuting players", "error")
    end
end)
