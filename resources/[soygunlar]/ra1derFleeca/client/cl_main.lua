...
RegisterNetEvent("ra1derFleeca:resetDoorState")
AddEventHandler("ra1derFleeca:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("ra1derFleeca:lootup_c")
AddEventHandler("ra1derFleeca:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("ra1derFleeca:outcome")
AddEventHandler("ra1derFleeca:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("ra1derFleeca:startheist", Fleeca.Banks[arg], arg)
    elseif not oc then
        QBCore.Functions.Notify(arg)
    end
end)

RegisterNetEvent("ra1derFleeca:startLoot_c")
AddEventHandler("ra1derFleeca:startLoot_c", function(data, name)
    currentname = name
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot then
                        ...
                        if dst1 < 2 then
                            ...
                            exports['qb-ui']:showInteraction('<b>[E]</b> Collect','left')
                                helpTextShowing = true
                            elseif dst1 < 0.75 and IsControlJustReleased(0, 38) then
                                exports['qb-ui']:hideInteraction()
                                TriggerServerEvent("ra1derFleeca:lootup", name, "Loot")
                                StartGrab(name)
                                LootCheck[name].Stop = true
                                startdstcheck = false
                                TriggerServerEvent("ra1derFleeca:stopHeist", name)
                            end
                        else   
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("ra1derFleeca:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("ra1derFleeca:stopHeist_c")
AddEventHandler("ra1derFleeca:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("ra1derFleeca:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("ra1derFleeca:toggleVault")
AddEventHandler("ra1derFleeca:toggleVault", function(key, state)
    ...
    if Fleeca.Banks[key].hash == nil then
        if not state then
            ...
            TriggerServerEvent("ra1derFleeca:updateVaultState", key, Doors[key][1].state)
        elseif state then
            local obj = GetClosestObjectOfType(Fleeca.Banks[key].doors.startloc.x, Fleeca.Banks[key].doors.startloc.y, Fleeca.Banks[key].doors.startloc.z, 2.0, GetHashKey(Fleeca.vaultdoor), false, false, false)
            local count = 0
...
AddEventHandler("ra1derFleeca:reset", function(name, data)
    for i = 1, #LootCheck[name], 1 do
        LootCheck[name][i] = false
    end
    Check[name] = false
    QBCore.Functions.Notify("CASE DOOR CLOSES AFTER 5 SECONDS!", "error")
    Citizen.Wait(5000)
    QBCore.Functions.Notify("THE CASE DOOR IS CLOSING", "error")
    TriggerServerEvent("ra1derFleeca:toggleVault", name, true)
    TriggerEvent("ra1derFleeca:cleanUp", data, name)
end)


AddEventHandler("ra1derFleeca:startheist", function(data, name)
    disableinput = true
    currentname = name
    currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent("ra1derFleeca:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

AddEventHandler("ra1derFleeca:cleanUp", function(data, name)
    Citizen.Wait(10000)
    ...
    TriggerServerEvent("ra1derFleeca:setCooldown", name)
    initiator = false
end)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("ra1derFleeca:startLoot", data, name, missionplayers)
    done = false
end
...
function StartGrab(name)
    ...
    local CashAppear = function()
	    ...
	    Citizen.CreateThread(function()
		    while GetGameTimer() - startedGrabbing < 37000 do
			    ...
			    if HasAnimEventFired(ped, GetHashKey("CASH_APPEAR")) then
				    if not IsEntityVisible(grabobj) then
                        ...
                        TriggerServerEvent("ra1derFleeca:rewardCash")
				    end
			    end
            ...
            TriggerServerEvent("ra1derFleeca:giveCard")
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("ra1derFleeca:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('ra1derFleeca:darkmail')
AddEventHandler('ra1derFleeca:darkmail', function()
    Citizen.Wait(1000)
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = "Fleeca",
        subject = "Gizli Mesaj",
        message = "Well done!: <br><br>Hack Successful! </b><br><br> Hack systems come into play, after a short while the door will be triggered and opened.",
    })
end)

RegisterNetEvent('ra1derFleeca:UseGreenLapTop')
AddEventHandler('ra1derFleeca:UseGreenLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    QBCore.Functions.TriggerCallback("ra1derFleeca:getBanks", function(bank, door)
        Fleeca.Banks = bank
        Doors = door
    end)
	TriggerEvent("ra1derFleeca:freezeDoors")
                ...
                QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty)
                    if qtty > 0 then
                        QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty2)
                            if qtty2 > 0 then
                                QBCore.Functions.TriggerCallback('ra1derFleeca:startcheck', function(checkbro,bildirim)
                                    if checkbro then
                                        ...
                                        exports['varhack']:OpenHackingGame(function(success)
                                            if success then
                                                ...
                                                TriggerEvent('ra1derFleeca:darkmail')
                                                DeleteObject(laptop)
                                                ClearPedTasksImmediately(ply)  
                                                Citizen.SetTimeout(5000, function()
                                                    TriggerServerEvent("ra1derFleeca:start",k)
                                                end)  
                                            else
                                                StopAnimTask(PlayerPedId(), hackAnimDict, "hack_loop", 1.0)
                                                DeleteObject(laptop)
                                        ...
                                        QBCore.Functions.Notify(bildirim)
                                    end
                                end,k)
                            else
                                QBCore.Functions.Notify("You Don't Have a Security Card", "error")
                            end
                        end, "guvenlik_karti_d")
                    else
