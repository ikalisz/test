...
RegisterNetEvent("nx-spikes:client:usespikes", function()
    ...
    if not IsPedInAnyVehicle(ped, false) then
        if player.job.name == "police" then
            ...
            QBCore.Functions.Notify('you cant use it', 'error', 7500)
        end
    else
        QBCore.Functions.Notify("You cannot use Barbed Wire in a vehicle.", 'error', 7500)
    end
end)
    ...
    options = {
        {
            ...
            label = "Get Spikestrip",
            canInteract = function()
                local xPlayer = QBCore.Functions.GetPlayerData()
                if xPlayer.job.name == "police" then return true end
            end
        },
    },
    distance = 2.0
