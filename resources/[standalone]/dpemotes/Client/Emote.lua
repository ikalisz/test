...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(1, 21) and IsControlPressed(1, 182) then 
      PtfxStop()
...
-- Commands / Events --------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------
RegisterNetEvent('dpemotes:play-anim')
AddEventHandler('dpemotes:play-anim', function(args)
  EmoteCommandStart(args) 
end)



RegisterNetEvent('dpemotes:animation-menu')
AddEventHandler('dpemotes:animation-menu', function(args)
  OpenEmoteMenu()
end)
...
function EmoteChatMessage(args)
  if args == display then
    TriggerEvent('chatMessage', "Help", "normal", string.format(""))
  else
    TriggerEvent('chatMessage', "Help", "normal", string.format(""..args..""))
  end
end
...
local lastDamageTime = GetGameTimer()
Citizen.CreateThread(function() -- Create check for weapon type and apply stab(taze) if its a tazer
  while true do
    local ped = PlayerPedId()
    if HasPedBeenDamagedByWeapon(ped, 0, 2) then
      lastDamageTime = GetGameTimer() + 20000
...
function EmoteCommandStart(args)
  if GetGameTimer() > lastDamageTime then
    if #args > 0 then
      local name = string.lower(args[1])
      if name == "c" then
          if IsInAnimation then
              EmoteCancel()
              TriggerEvent("tgiann-fishing:break")
          else
    ...
    TriggerEvent("QBCore:Notify", "You Can't Animate For 20 Seconds Because You Take Damage!", "error")
  end
end
