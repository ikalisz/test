...
AddEventHandler('longboard:start', function()
	if not IsPedInAnyVehicle(PlayerPedId()) then RCCar.Start() end
end)

AddEventHandler('tgiann:playerdead', function(dead)
	if dead and Attached then
...
RCCar.HandleKeys = function(distanceCheck)
	if distanceCheck <= 1.5 then
		if not openUi then
			openUi = true
			SendNUIMessage({type = 'open', onBoard = false})
		end

		DisableControlAction(0, 38, true)
		DisableControlAction(0, 113, true)

		if IsDisabledControlJustPressed(0, 38)  then -- E / Yerden Alma
			RCCar.Attach("pick")
		end

		if IsDisabledControlJustPressed(0, 113) then -- G / İn Bin
			if Attached then
				RCCar.AttachPlayer(false)
	...
	if distanceCheck < Config.LoseConnectionDistance then
		...
		Citizen.CreateThread(function()
			...
			if Attached then
				...
				if RCCar.MustRagdoll() then
					...
					local RagdollTime = math.random(200,500)
		...
		if Attached then
			if not IsEntityPlayingAnim(player, "move_strafe@stealth", "idle", 1) and not IsDisabledControlPressed(0, 22) then
			...
			DisableControlAction(0, 22, true) -- Boşluk

			if IsDisabledControlPressed(0, 32) and not IsDisabledControlPressed(0, 33) and not overSpeed then 
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 9, 1)
			end

			if IsDisabledControlPressed(0, 22) and Attached then
				-- Jump system
				if not IsEntityInAir(RCCar.Entity) then	
					...
					local duration = 0
					local boost = 0
					while IsDisabledControlPressed(0, 22) do
						Citizen.Wait(10)
			...
			if IsDisabledControlPressed(0, 33) and not IsDisabledControlPressed(0, 32) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 22, 1)
			end

			if IsDisabledControlPressed(0, 34) and IsDisabledControlPressed(0, 33) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 13, 1)
			end

			if IsDisabledControlPressed(0, 35) and IsDisabledControlPressed(0, 33) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 14, 1)
			end

			if IsDisabledControlPressed(0, 32) and IsDisabledControlPressed(0, 33) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 30, 100)
			end

			if IsDisabledControlPressed(0, 34) and IsDisabledControlPressed(0, 32) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 7, 1)
			end

			if IsDisabledControlPressed(0, 35) and IsDisabledControlPressed(0, 32) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 8, 1)
			end

			if IsDisabledControlPressed(0, 34) and not IsDisabledControlPressed(0, 32) and not IsDisabledControlPressed(0, 33) and not overSpeed then
				TaskVehicleTempAction(RCCar.Driver, RCCar.Entity, 4, 1)
			end

			if IsDisabledControlPressed(0, 35) and not IsDisabledControlPressed(0, 32) and not IsDisabledControlPressed(0, 33) and not overSpeed then
...
RCCar.Spawn = function()
	-- models to load
	RCCar.LoadModels({ `bmx`, 68070371, `p_defilied_ragdoll_01_s`, "pickup_object", "move_strafe@stealth", "move_crouch_proto"})
	...
	RCCar.Entity = CreateVehicle(`bmx`, spawnCoords, spawnHeading, true)
	RCCar.Skate = CreateObject(`p_defilied_ragdoll_01_s`, 0.0, 0.0, 0.0, true, true, true)
	...
	DisablePedPainAudio(RCCar.Driver, true)
...
RCCar.Attach = function(param)
	...
	elseif param == "pick" then
		...
		-- Clear 
		RCCar.Clear()
		TriggerServerEvent("electric-skateboard:item", false)
		SendNUIMessage({type = 'close'})
	end
end
...
RCCar.AttachPlayer = function(toggle)
	if toggle then
		AttachEntityToEntity(RCCar.Skate, RCCar.Entity, GetPedBoneIndex(PlayerPedId(), 28422), 0.0, 0.0, -0.40, 0.0, 0.0, 90.0, false, true, true, true, 1, true)
		SetVehicleOnGroundProperly(RCCar.Entity)
		QBCore.Functions.Progressbar("kaykay", "Skateboard being dropped down!", 1500, false, false, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
			...
			Attached = toggle
			if not IsEntityPlayingAnim(player, "move_strafe@stealth", "idle", 1) then
				TaskPlayAnim(player, "move_strafe@stealth", "idle", 8.0, 8.0, -1, 1, 1.0, false, false, false)
			end
			...
			Citizen.Wait(500)
			SendNUIMessage({type = 'open', onBoard = true})
		end, function() -- Cancel
			RCCar.AttachPlayer(false)
		end)
