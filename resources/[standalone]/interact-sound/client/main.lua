...
AddEventHandler('InteractSound_CL:PlayOnOne', function(soundFile, soundVolume, ikili)
    if ikili == nil then
        ikili = false
    end
    SendNUIMessage({
...
AddEventHandler('InteractSound_CL:PlayWithinDistance', function(playerNetId, maxDistance, soundFile, soundVolume)
    ...
    local playerIndex = GetPlayerFromServerId(playerNetId)
    local pedCoords = GetEntityCoords(GetPlayerPed(playerIndex))
    if playerIndex ~= -1 then
        local distIs  = Vdist(lCoords.x, lCoords.y, lCoords.z, pedCoords.x, pedCoords.y, pedCoords.z)
...
RegisterNetEvent('InteractSound_CL:PlayCoords')
AddEventHandler('InteractSound_CL:PlayCoords', function(coords, maxDistance, soundFile, soundVolume)
    local lCoords = GetEntityCoords(PlayerPedId())
