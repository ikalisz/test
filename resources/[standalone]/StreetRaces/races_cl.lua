...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/race2', 'Start Racing and Prep.', {{ name="Race Commands", help="clean, leave,record,save,delete,list,load,start,cancel"}})

    TriggerEvent('chat:addSuggestion', '/race2 clean', "Delete Checkpoints from Map")

    TriggerEvent('chat:addSuggestion', '/race2 leave', 'Leave the unstarted race joined.')

    TriggerEvent('chat:addSuggestion', '/race2 record', 'Set Race with Checkpoint System from Map (You Can Set Checkpoint by Left Clicking on the Map Twice)')

    TriggerEvent('chat:addSuggestion', '/race2 save', 'Save Set Race', {{ name="Registration Name", help="Give Your Race a Name"}})

    TriggerEvent('chat:addSuggestion', '/race2 delete', 'delete a Saved Race', {{ name="Race Name", help="Name of Saved Race"}})

    TriggerEvent('chat:addSuggestion', '/race2 load', 'Upload a Saved Race', {{ name="Race Name", help="Name of Saved Race"}})

    TriggerEvent('chat:addSuggestion', '/race2 list', 'List Saved Races')

    TriggerEvent('chat:addSuggestion', '/race2 start ', 'Marked or Loaded Start Race', {{ name="Bet Amount", help="Participation Fee"}, { name="Start Time", help="After How Many Seconds Will The Race Begin?"}})

    TriggerEvent('chat:addSuggestion', '/race2 cancel ', 'Cancel Started Race')
end)

-- Main command for races
RegisterCommand("race2", function(source, args)
    if args[1] == "clean" or args[1] == "leave" then
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("~g~%d~w~ In Seconds The Race Will Begin"):format(math.ceil(count/1000.0)))
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "you joined!")
                end
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("~g~$%d~w~ Join the Race in return. Remaining Time: ~y~%d~w~s"):format(race.amount, count))
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "[~g~E~w~] Join the Race")
