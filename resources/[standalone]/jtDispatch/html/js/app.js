...
function close() {
    ...
    $.post('http://jtDispatch/close', JSON.stringify({}));
}
...
function createNotification(data) {
    ...
    var blink_me = data.code == "KOD 99" || data.code == "KOD 0" || data.code == "KOD 66" ? 'notification-item blink_me' : 'notification-item'
    ...
    let elem = $('<div/>', {
        ...
        <div class="notification-header">
            <div>
                <span id="data_id_span">${data.ihbarid}</span>
                <span id="data_code_span">${data.code}</span>
                <span id="data_desc_span">${data.description}</span>
            </div>
            <span></span>
        </div>
        ...
        ${data.coords != undefined ? `
            <div class="location-field">
                <i id="set-waypoint" data-coords-x=${data.coords.x.toFixed(2)} data-coords-y=${data.coords.y.toFixed(2)} class="fas fa-map-marker-alt"></i>
                <span id="shortcut">[E]</span>
            ...
            ` : ""
             ...
             <div class="clock-field">
                ...
                ${data.vehicle!==undefined && data.plate!==undefined ? ` 
                <div class="vehicle-plate">
                <i class="fas fa-car"></i>
                <span>${data.vehicle} - ${data.plate}</span>
                </div>
            `: ""}
    ...
    $(elem).find('#set-waypoint').click(function() {
       $.post('http://jtDispatch/set-waypoint', JSON.stringify({x: $(this).attr('data-coords-x'), y: $(this).attr('data-coords-y')})); 
    });

    $(elem).find('#button-icon').click(function() {
        if ($(elem).find('#button-icon').hasClass("fa-plus-circle")) {
            $.post('http://jtDispatch/addActiveUnit', JSON.stringify({citizenid: Self.citizenId, notifid: data.id, radioid: Self.radioCode}));
        } else {
            $.post('http://jtDispatch/removeActiveUnit', JSON.stringify({citizenid: Self.citizenId, notifid: data.id, radioid: Self.radioCode}));
        }
    });
...
$("#radio-code").keyup(function(event) { 
    if (event.which == 13) {
        ...
        $.post("http://jtDispatch/set-radio-code", JSON.stringify({radiocode}), (data) => {
            if (data) {
                if (data) {
