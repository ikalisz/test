...
AddEventHandler('Drones:Buy', function(drone)
    ...
    if xPlayer.Functions.RemoveMoney(Config.BankAccountName, drone.price) then
    	TriggerClientEvent("QBCore:Notify", xPlayer.PlayerData.source, 'The drone was purchased successfully')
    	xPlayer.Functions.AddItem(drone.name, 1)
    	TriggerClientEvent("Drones:CloseUI", src)
    else
    	TriggerClientEvent("QBCore:Notify", xPlayer.PlayerData.source, 'You dont have enough money in your bank to buy this drone')
    end
end)
