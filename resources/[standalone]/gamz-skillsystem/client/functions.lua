...
-- RegisterCommand("skillfulle", function()
--     UpdateSkill("Sürüş Yeteneği", 100.0)
--     UpdateSkill("Ciğer Kapasitesi", 100.0)
--     UpdateSkill("Kondisyon", 100.0)
--     UpdateSkill("Ön Kaldırma Yeteneği", 100.0)
--     UpdateSkill("Güç", 100.0)
-- end, false)
...
RefreshSkills = function()
    for type, value in pairs(Config.Skills) do
        ...
        if QBCore.UI.Menu.IsOpen("default", GetCurrentResourceName(), "skill_menu") then
            SkillMenu()
        end
    end
end
                ...
                if GetVehicleClass(vehicle) == 8 or GetVehicleClass(vehicle) == 13 and speed >= 5 then
                    ...
                    if IsControlPressed(0, 210) then
                        if rotation.x >= 25.0 then
                            UpdateSkill("Front Lift Capability", 0.5)
                        end 
                    end
                end

                if speed >= 80 then
                    UpdateSkill("Driving Ability", 0.1)
                end
            end
		end
	end
...
Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/skill', 'Open Skill Menu Close', {})
end)
