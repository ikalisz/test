...
QBCore.Functions.Notify('You Saved the Song', 'success', 7500)
    else
        QBCore.Functions.Notify('Song Save Failed', 'error', 7500)
    end
end)
...
AddEventHandler('wasabi_boombox:deleteSong', function(data)
	local confirmed = lib.alertDialog({
		header = 'Delete Song',
		content = 'Are you sure you want to delete the song??',
		centered = true,
		cancel = true
	})
	if confirmed == 'Approved' then
		TriggerServerEvent('wasabi_boombox:deleteSong', data)
        QBCore.Functions.Notify('Couldnt Delete Song', 'error', 7500)
	else
        QBCore.Functions.Notify('Song Deleted', 'success', 7500)
        -- TriggerServerEvent('wasabi_boombox:deleteSong', data)
	end
end)
...
AddEventHandler('wasabi_boombox:playMenu', function(data)
    ...
    if data.type == 'play' then
        local keyboard = lib.inputDialog('Play Song', {'Link','Audio Distance (Max 40)', 'Volume (1-100)'})
        if keyboard then
    ...
    elseif data.type == 'distance' then
        local keyboard = lib.inputDialog('Change Audio Distance', {'Distance (Max 40)'})
        if keyboard then
