...
exports['qb-target']:AddBoxZone("pdbooss", vector3(-1381.80, -616.547, 31.8), 0.4, 0.9, {
    heading=126.59,
    minZ=30.73,
    maxZ=32.46,
	debugPoly = false 
...
RegisterNUICallback('down', function(data, cb)
    ...
    if currentDJ.volume <= 0.0 then
        QBCore.Functions.Notify('Youve reached the minimum volume', 'error', 7500)
    end
end)
...
RegisterNUICallback('up', function(data, cb)
    ...
    if currentDJ.volume >= 1.0 then
        QBCore.Functions.Notify('You have reached the maximum volume', 'error', 7500)
    end
end)
...
RegisterNUICallback('noSongtitle', function(data, cb)
	...
	QBCore.Functions.Notify('This Title Is Not Available!', 'error', 7500)
end)
