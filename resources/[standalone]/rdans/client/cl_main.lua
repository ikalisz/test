...
RegisterCommand('dance', function(src, args, raw)
    local randomdances = math.random(1, #animations)
    if args[1] == nil then
        TriggerEvent('qy:dances:dance', randomdances)
        QBCore.Functions.Notify("Dance "..randomdances)
    else
        TriggerEvent('qy:dances:dance', tonumber(args[1]))
        QBCore.Functions.Notify("Dance "..args[1])
    end
end)

local currentDance = 0
RegisterNetEvent('qy:dances:dance', function(pDance)
    local noAnimations = #animations

    if pDance == currentDance then
        ClearPedTasks(PlayerPedId())
        return ClearPedTasks
    end
