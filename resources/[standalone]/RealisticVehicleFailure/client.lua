...
local function disableVehicle()
	...
	for i=1, #cfg.disableVehicleTable do
		if GetEntityModel(vehicle) == cfg.disableVehicleTable[i] then
			return false
		end
	end
	return true
...
RegisterNetEvent('iens:motortamiret')
AddEventHandler('iens:motortamiret', function(vehicle, bilgi)
	healthEngineLast = bilgi
	SetVehicleEngineHealth(vehicle, bilgi)
end)

RegisterNetEvent('nitro:motorhealt')
AddEventHandler('nitro:motorhealt', function(vehicle, bilgi)
	healthEngineLast = bilgi
	SetVehicleEngineHealth(vehicle, bilgi)
end)
		...
		if isPedDrivingAVehicle() then
			if disableVehicle() then
				if pedInSameVehicleLast then
					...
					if GetVehicleClass(vehicle) ~= 14 then -- Not for boats
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		local ped = PlayerPedId()
		if isPedDrivingAVehicle() and disableVehicle() then
			vehicle = GetVehiclePedIsIn(ped, false)
			vehicleClass = GetVehicleClass(vehicle)
			healthEngineCurrent = GetVehicleEngineHealth(vehicle)
			if healthEngineCurrent == 1000 then healthEngineLast = 1000.0 end
			if healthEngineLast == nil then healthEngineLast = 1000.0 end
			healthEngineNew = healthEngineCurrent
			healthEngineDelta = healthEngineLast - healthEngineCurrent
			local dmgMultiplier = cfg.classDamageMultiplier[vehicleClass]
			if dmgMultiplier == nil then 
				dmgMultiplier = 1.0
				cfg.classDamageMultiplier[vehicleClass] = dmgMultiplier
			end
			healthEngineDeltaScaled = healthEngineDelta * cfg.damageFactorEngine * dmgMultiplier

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * cfg.damageFactorBody * dmgMultiplier
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * cfg.damageFactorPetrolTank * dmgMultiplier
