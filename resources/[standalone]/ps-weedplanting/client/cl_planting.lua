...
RegisterCommand("weedloaded", function()
...
RegisterNetEvent('ps-weedplanting:client:UseWeedSeed', function()
    ...
    exports['qb-ui']:showInteraction(_U('place_or_cancel'), 'left')
    ...
    while not planted do
        ...
        if hit == 1 then
            ...
            if IsControlJustPressed(0, 38) then
                ...
                QBCore.Functions.Progressbar('spawn_plant', _U('place_sapling'), 20000, false, true, {
        ...
        elseif result.growth == 100 then -- Harvest
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Aşama: ' .. result.stage .. ' - Sağlık: ' .. result.health,
        ...
        elseif result.gender == 'female' then -- Option to add male seed
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Büyüme: ' .. result.growth .. '%' .. ' - Aşama: ' .. result.stage,
                        txt = 'Sağlık: ' .. result.health,
                ...
                exports['qb-menu']:openMenu({
                    ...
                    {
                        header = 'Gübre: ' .. result.fertilizer .. '%',
...
RegisterNetEvent('ps-weedplanting:client:ClearPlant', function(entity)
    ...
    QBCore.Functions.Progressbar('clear_plant', _U('clear_plant'), 20000, false, true, {
        disableMovement = true,
...
RegisterNetEvent('ps-weedplanting:client:HarvestPlant', function(entity)
    ...
    QBCore.Functions.Progressbar('harvest_plant', _U('harvesting_plant'), 15000, false, true, {
        disableMovement = true, 
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('ps-weedplanting:client:PoliceDestroy', function(entity)
    ...
    if Player.job.name == 'police' then 
        ...
        QBCore.Functions.Notify('Polis değilsin mal', 'error', 7500)
    end
end)
...
RegisterNetEvent('ps-weedplanting:client:FireGoBrrrrrrr', function(coords)
    ...
    QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty)
        if qtty > 0 then
            QBCore.Functions.Progressbar('name', 'Bitki Yakılıyor', 5000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
            disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
            ...
            QBCore.Functions.Notify('Çakmağın Yok!', 'error', 7500)
        end
    end, "lighter")
end)
...
RegisterNetEvent('ps-weedplanting:client:GiveWater', function(entity)
    QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty)
        if qtty > 0 then
            ...
            QBCore.Functions.Progressbar('weedplanting_water', _U('watering_plant'), 10000, false, false, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
