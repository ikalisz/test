...
local function SetupInstructions()
    ...
    exports['qb-ui']:showInteraction("[E] Leave the Telescope", nil, "ayril")
...
local function UseTelescope(entity)
    ...
    QBCore.Functions.Notify('Someone is already using the telescope', 'error', 7500)
        return
    end
    ...
    elseif dist > 2.0 then
        QBCore.Functions.Notify('No Telescope Nearby', 'error', 7500)
        ClearPedTasks(playerPed)
        return
    end
            ...
            if not inTelescope then
                ...
                if closest ~= 0 then
                    if distance < Config.MaxInteractionDist then
                        ...
                        openUi = true
                        arayuz = true
                        exports['qb-ui']:showInteraction("[E] Telescope", nil, "31")
                        if IsControlJustPressed(0, 38) then
                            local telescope = GetClosestTelescope()
                            if telescope ~= 0 then
                                UseTelescope(telescope)
                                exports['qb-ui']:hideInteraction("31")
                            else
                                QBCore.Functions.Notify('Telescope Not Found', 'error', 7500)
                            end
                        end
                    else
                        sleep = distance*20
