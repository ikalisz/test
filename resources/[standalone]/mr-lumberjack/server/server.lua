...
RegisterNetEvent('mr-lumberjack:BuyAxe', function()
    ...
    local otuzbir = Config.LumberaxePrice
    local axe = Player.Functions.GetItemByName("weapon_battleaxe").amount > 0 
    if not axe then
        ...
        Player.Functions.RemoveMoney("cash", otuzbir)
...
RegisterServerEvent('mr-lumberjack:recivelumber', function()
    ...
    Player.Functions.AddItem('odun', lumber)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['odun'], "add")
end)
