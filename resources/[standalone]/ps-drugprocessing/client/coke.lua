			...
			if qtty >= 1 then
				QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty2)
					...
					TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_PARKING_METER", 0, true)
						...
						QBCore.Functions.Notify("Kokain Yaprağın Yok!", 'error')
					end
				end, "coca_leaf")
			else
				QBCore.Functions.Notify("Kırpma Makasın Yok!", 'error')
			end
					...
					if qtty2 >= 1 then
						QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty3)
							if qtty3 >= 10 then
								...
								QBCore.Functions.Notify("Kokain Yok?", 'error')
							end
						end, "coke")
					else
						QBCore.Functions.Notify("İnce Ölçek Yok?", 'error')
					end
				end, "finescale")
			else
				QBCore.Functions.Notify("Yeterli Sayıda Karbonat Yok", 'error')
			end
		end, "bakingsoda")
		else
		...
		if not isProcessing then
			QBCore.Functions.TriggerCallback('elixir-base:item-kontrol', function(qtty)
				if qtty >= 1 then
					...
					QBCore.Functions.Notify("Yeterli sayıda kokain paketin yok!", 'error')
				end
