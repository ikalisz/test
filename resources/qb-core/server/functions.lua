...
QBCore.Functions.kickHacKer = function(source, key)
	if key then
		if key ~= QBCore.Key then
			TriggerClientEvent("tgiann-hackkoruma:client:kick", source, "Tricked Event Run[Key Incorrect]")
			return false -- false
		end
		return true
	else
		TriggerClientEvent("tgiann-hackkoruma:client:kick", source, "Tricked Event [Key Not Found]")
		return false
	end
end

QBCore.Functions.CreateCallback("qb-core:check-key", function(source, cb)
