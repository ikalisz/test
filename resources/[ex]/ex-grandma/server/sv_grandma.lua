...
RegisterServerEvent('ex-grandma:server:checkfunds', function()
    local src = source
    ...
    if CheckBalance then -- Checks for money balance
        if balance >= Config.Cost then -- Checks to make sure player has enough funds
            TriggerClientEvent('ex-grandma:reviveplayer', source) -- Starts Progressbar for Reviving
        else
        ...
        TriggerClientEvent('ex-grandma:reviveplayer', source)
    end
end)
