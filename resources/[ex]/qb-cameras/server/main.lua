    ...
    if FetchedData[numid] ~= nil then 
        ...
        if dt ~= nil then 
            if dt.isOnline ~= data.enable then
                ...
                if data.enable then 
                    TriggerClientEvent("QBCore:Notify", src, "Camera is now active.", "success")
                else
                    TriggerClientEvent("QBCore:Notify", src, "Camera has been deactivated for a short time.", "error")
                end
            else
                if data.enable then 
                    TriggerClientEvent("QBCore:Notify", src, "Camera is already functional.", "success")
                else
                    TriggerClientEvent("QBCore:Notify", src, "Camera is already turned off.", "error")
                end
            end
        end
    end
...
AddEventHandler("qb-cameras:server:RemoveCamera", function(data)
    ...
    if FetchedData[numid] ~= nil then 
        ...
        TriggerClientEvent("QBCore:Notify", src, "Camera disconnected and added to your bag.", "success")
    end
end)
