...
RegisterCommand("adminloaded", function()
    PlayerData = QBCore.Functions.GetPlayerData()
end, true)


RegisterKeyMapping('+adminmenu', 'Activates the admin menu', 'keyboard', 'pageup')
Citizen.CreateThread(function()
    RegisterKeyMapping('+noclip', 'Noclip', 'keyboard', 'pagedown')
end)
...
RegisterNetEvent("noclip:open", function()
    QBCore.Functions.TriggerCallback('qb-admin:server:getrank', function(rank)
        if rank then
            SendNUIMessage({
                Action = 'Close',
...
-- RegisterCommand("+noclip", function()
--     ToggleNoClipMode()
--   --  TriggerEvent('chatMessage', "Noclip", "primary", "Noclip açıldı, PageUP tuşu ile kapatabilirsin, Q Tuşu ile yukarı çıkabilir, Z tuşu ile aşağı inebilirsin. SHIFT Tuşu ile hızlanabilirsin")
-- end)
...
RegisterNetEvent('qb-adminmenu:kill', function()
    SetEntityHealth(PlayerPedId(), 0)
    QBCore.Functions.Notify("Öldürüldün veya Öldün", "error")
end)
...
noclip_button:On('change', function(item, newValue, oldValue)
    ...
    TriggerEvent("noclip:open")
    -- TriggerClientEvent('DiscordBot:ToDiscord', 'adminlog', "İsimli Admin ``NOCLİP AÇTI`` ", PlayerData.PlayerData.source)
end)
...
menu12_button2:On('Select', function(item)
    ...
    ExecuteCommand("carfix", "fix", {})
end)



RegisterCommand("exitmenu", function(menu)
    MenuV:CloseMenu(menu)
end)

menu12_button4:On('Select', function(item)
    ExecuteCommand("dv", "dv", {})
end)
