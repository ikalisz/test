...
AddEventHandler('qb-houses:client:addGarage', function()
    if closesthouse ~= nil then 
        ...
        QBCore.Functions.Notify("Etrafta ev yok tırrek..", "error")
    end
end)
    ...
    if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
        if hasKey then
            if Config.Houses[closesthouse].locked then
                ...
                QBCore.Functions.Notify("Evin kilidi acildi!", "success", 2500)
            else
                TriggerServerEvent('qb-houses:server:lockHouse', true, closesthouse)
                QBCore.Functions.Notify("Ev kilitli!", "error", 2500)
            end
        else
            QBCore.Functions.Notify("Bu evin anahtarları sende yok...", "error", 3500)
        end
    else
        QBCore.Functions.Notify("Görünürde kapı yok??", "error", 3500)
    end
end)
            ...
            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) < 30)then
                ...
                if hasKey then
                    ...
                    if not inside then
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                QBCore.Functions.DrawText3D(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '~b~/enter~w~ - ile eve gir')
                            end
                        end
                    end
                    ...
                    if CurrentDoorBell ~= 0 then
                        if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                            QBCore.Functions.DrawText3D(Config.Houses[closesthouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[closesthouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[closesthouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z + 0.35, 'Kapıyı Açmak İçin [~g~G~w~] Bas')
                            ...
                            if POIOffsets ~= nil then
                                if POIOffsets.exit ~= nil then
                                    if(GetDistanceBetweenCoords(pos, Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                                        QBCore.Functions.DrawText3D(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, '[E] - Evden Çık')
                                        QBCore.Functions.DrawText3D(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z - 0.2, '[H] - Kapı Kamerasına Bak')
                        ...
                        if closesthouse ~= nil then
                            if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                if not viewCam and Config.Houses[closesthouse].locked then
                                    QBCore.Functions.DrawText3D(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '[~g~E~w~] - Evi İncele')
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        TriggerServerEvent('qb-houses:server:viewHouse', closesthouse)
                                    end
                                -- elseif not viewCam and not Config.Houses[closesthouse].locked then
                                --     QBCore.Functions.DrawText3D(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, '[~g~E~w~] Om naar ~b~binnen~w~ te gaan')
                                --     if IsControlJustPressed(0, Keys["E"])  then
                                --         enterNonOwnedHouse(closesthouse)
                                --     end
                                end
                        ...
                        if closesthouse ~= nil then
                            if not inOwned then
                                -- if(GetDistanceBetweenCoords(pos, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, true) < 1.5)then
                                    ...
                                    --     end
                                    -- else
                                    --     QBCore.Functions.DrawText3D(Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, 'De deur is ~r~vergrendeld / ~g~G~w~ - Aanbellen')
                                    --     if IsControlJustPressed(0, Keys["G"]) then
                                    --         TriggerServerEvent('qb-houses:server:RingDoor', closesthouse)
                                    --     end
                                    -- end
                            ...
                            elseif inOwned then
                                if POIOffsets ~= nil then
                                    if POIOffsets.exit ~= nil then
                                        if(GetDistanceBetweenCoords(pos, Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, true) < 1.5)then
                                            QBCore.Functions.DrawText3D(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z, '[~r~E~w~] - Evden Ayrıl')
                                            if IsControlJustPressed(0, Keys["E"]) then
                    ...
                    if CurrentHouse ~= nil then
                        if stashLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, stashLocation.x, stashLocation.y, stashLocation.z, true) < 1.5)then
                                QBCore.Functions.DrawText3D(stashLocation.x, stashLocation.y, stashLocation.z, '~g~[E]~w~ - Depo')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    print(CurrentHouse)
                                    TriggerEvent("inventory:client:SetCurrentStash", CurrentHouse, QBCore.Key)
                            ...
                            elseif(GetDistanceBetweenCoords(pos, stashLocation.x, stashLocation.y, stashLocation.z, true) < 3)then
                                QBCore.Functions.DrawText3D(stashLocation.x, stashLocation.y, stashLocation.z, 'Depo')
                            end
                        end
                    end
                end
                    ...
                    if CurrentHouse ~= nil then
                        if outfitLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 1.5)then
                                QBCore.Functions.DrawText3D(outfitLocation.x, outfitLocation.y, outfitLocation.z, '[~g~E~w~] - Wardrobe')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerEvent('qb-clothing:client:openOutfitMenu')
                                end
                            elseif(GetDistanceBetweenCoords(pos, outfitLocation.x, outfitLocation.y, outfitLocation.z, true) < 3)then
                                QBCore.Functions.DrawText3D(outfitLocation.x, outfitLocation.y, outfitLocation.z, 'Wardrobe')
                            end
                        end
                    end
                end
                    ...
                    if CurrentHouse ~= nil then
                        if logoutLocation ~= nil then
                            if(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 1.5)then
                                QBCore.Functions.DrawText3D(logoutLocation.x, logoutLocation.y, logoutLocation.z, '[~g~E~w~] - Exit')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    DoScreenFadeOut(250)
                            ...
                            elseif(GetDistanceBetweenCoords(pos, logoutLocation.x, logoutLocation.y, logoutLocation.z, true) < 3)then
                                QBCore.Functions.DrawText3D(logoutLocation.x, logoutLocation.y, logoutLocation.z, 'Exit')
                            end
                        end
                    end
                end
...
AddEventHandler('qb-houses:client:RingDoor', function(player, house)
    if closesthouse == house and inside then
        ...
        QBCore.Functions.Notify("Someone is knocking on the door!!")
    end
end)
...
AddEventHandler('qb-houses:client:giveHouseKey', function(data)
    ...
    if player ~= -1 and distance < 2.5 and closesthouse ~= nil then
        ...
        if housedist < 10 then
            ...
            QBCore.Functions.Notify("Eand you're not close enough..", "error")
        end
    elseif closesthouse == nil then
        QBCore.Functions.Notify("No houses nearby!", "error")
    else
        ...
        if housedist < 5 then
            QBCore.Functions.TriggerCallback('qb-houses:server:getHouseOwner', function(result)
                if QBCore.Functions.GetPlayerData().citizenid == result then
                    ...
                    QBCore.Functions.Notify("Don't own a house..", "error")
                end
            end, closesthouse)
        else
            QBCore.Functions.Notify("You're not close enough to home..", "error")
        end
    else
        QBCore.Functions.Notify("You're not close enough to home..", "error")
    end
...
function HouseKeysMenu()
    ...
    QBCore.Functions.TriggerCallback('qb-houses:server:getHouseKeyHolders', function(holders)
        ...
        if holders == nil or next(holders) == nil then
            QBCore.Functions.Notify("Key owner not found..", "error", 3500)
            closeMenuFull()
        else
...
function removeOutfit(oData)
    ...
    QBCore.Functions.Notify(oData.outfitname.." Deleted", "success", 2500)
    closeMenuFull()
end
...
function enterOwnedHouse(house)
    ...
    setHouseLocations()
    Citizen.Wait(1000)
    local koordamk = GetEntityCoords(PlayerPedId())
    exports['qb-target']:AddBoxZone("evislemleri", vector3(koordamk.x, koordamk.y, koordamk.z), 40, 40, {
        name = "evislemleri",
        heading = 11.0,
        debugPoly = false,
        minZ = (koordamk.z) - 6,
...
function leaveOwnedHouse(house)
    if not FrontCam then
        exports['qb-target']:RemoveZone("evislemleri")
        inside = false
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.25)
        openHouseAnim()
        Citizen.Wait(250)
...
function CreateInstuctionScaleform(scaleform)
    ...
    InstructionButtonMessage("Kamerayı Kapatmak İcin")
    PopScaleformMovieFunctionVoid()
...
RegisterNetEvent('stash:ac')
AddEventHandler('stash:ac', function()
    TriggerEvent('qb-houses:client:setLocation', 'setstash')
end)

RegisterNetEvent('kiyafet:ac')
AddEventHandler('kiyafet:ac', function()
...
AddEventHandler('qb-houses:client:setLocation', function(data)
    ...
    if inside then
        if hasKey then
            ...
            QBCore.Functions.Notify('Ev sahibi degilsin..', 'error')
        end
    else    
        QBCore.Functions.Notify('Ev icinde degilsin..', 'error')
    end
end)
                        ...
                        if not Config.Houses[closesthouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    TriggerServerEvent('qb-houses:server:lockHouse', false, closesthouse)
                                    QBCore.Functions.Notify('It worked, now the door is open.', 'success')
                                ...
                                QBCore.Functions.Notify('Failed. Please try again', 'error')
                                DoRamAnimation(false)
                            end)
                            TriggerServerEvent('qb-houses:server:SetRamState', true, closesthouse)
                        else
                            QBCore.Functions.Notify('Someones already working on the door..', 'error')
                        end
                    else
                        QBCore.Functions.Notify('The house is already open old man..', 'error')
                    end
                else
                    QBCore.Functions.Notify('Not close to a house..', 'error')
                end
    else
        QBCore.Functions.Notify('You are not near a house..', 'error')
    end
end)
...
AddEventHandler('qb-houses:client:ResetHouse', function()
    ...
    if closesthouse ~= nil then
        ...
        if Config.Houses[closesthouse].IsRammed then
            ...
            QBCore.Functions.Notify('This door is not broken, it is open..', 'error')
        end
    end
end)