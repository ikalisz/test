...
AddEventHandler('qb-houses:server:addNewHouse', function(street, coords, price, tier)
	...
	TriggerClientEvent("qb-houses:client:setHouseConfig", -1, Config.Houses)
	TriggerClientEvent('QBCore:Notify', src, "you added a house: "..label)
end)
...
AddEventHandler('qb-houses:server:buyHouse', function(house)
	...
	if (bankBalance >= HousePrice) then
		...
		TriggerEvent('DiscordBot:ToDiscord', 'evsatinal', "İsimli Oyuncu " ..HousePrice.. "$ Fiyatında bir ev ``Satın Aldı`` Ev Konumu : "..house.." ", pData.PlayerData.source)
	else
...
function getOfflinePlayerData(citizenid)
	exports.oxmysql:execute("SELECT `charinfo` FROM `players` WHERE `citizenid` = '"..citizenid.."'", function(result)
		Citizen.Wait(100)
		if result[1] ~= nil then 
			local charinfo = json.decode(result[1].charinfo)
			return charinfo
...
AddEventHandler('qb-houses:server:removeHouseKey', function(house, citizenData)
	...
	TriggerClientEvent('QBCore:Notify', src, citizenData.firstname .. " " .. citizenData.lastname .. "' Removed the keys..", 'error', 3500)
	QBCore.Functions.ExecuteSql(false, "UPDATE `player_houses` SET `keyholders` = '"..json.encode(housekeyholders[house]).."' WHERE `house` = '"..house.."'")
end)
...
QBCore.Functions.CreateCallback('qb-houses:server:getOwnedHouses', function(source, cb)
	...
	if pData then
		exports.oxmysql:execute('SELECT * FROM player_houses WHERE identifier = @identifier AND citizenid = @citizenid', {['@identifier'] = pData.PlayerData.steam, ['@citizenid'] = pData.PlayerData.citizenid}, function(houses)
			local ownedHouses = {}
...
QBCore.Functions.CreateCallback('qb-houses:server:getSavedOutfits', function(source, cb)
	...
	if pData then
		exports.oxmysql:execute('SELECT * FROM player_outfits WHERE citizenid = @citizenid', {['@citizenid'] = pData.PlayerData.citizenid}, function(result)
			if result[1] ~= nil then
				cb(result)
			else
				cb(nil)
...
AddEventHandler('qb-houses:server:giveHouseKey', function(target, house)
	...
	if tPlayer ~= nil then
		if housekeyholders[house] ~= nil then
			...
			TriggerClientEvent('QBCore:Notify', tPlayer.PlayerData.source, 'You have the keys. Home address: '..Config.Houses[house].adress..' !', 'success', 2500)
		else
			...
			QBCore.Functions.ExecuteSql(true, "UPDATE `player_houses` SET `keyholders` = '"..json.encode(housekeyholders[house]).."' WHERE `house` = '"..house.."'")
			TriggerClientEvent('qb-houses:client:refreshHouse', tPlayer.PlayerData.source)
			TriggerClientEvent('QBCore:Notify', tPlayer.PlayerData.source, 'You have keys.Home address: '..Config.Houses[house].adress..' !', 'success', 2500)
		end
	else
		TriggerClientEvent('QBCore:Notify', src, 'Something went upside down.Try again!', 'error', 2500)
	end
end)
...
QBCore.Commands.Add("createhouse", "ev olustur", {{name="price", help="Price"},{name="tier", help="seviye"}}, true, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add("addgarage", "garaj ekle", {}, false, function(source, args)
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.job.name == "realestate" then
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, item)
	...
	if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
		...
		TriggerClientEvent('QBCore:Notify', source, "Don't be a cops, Its just for cops", "error")
	end
end)
...
QBCore.Commands.Add("enter", "enter the house", {}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
