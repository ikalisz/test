...
CreateThread(function()--tunner
    exports["qb-polyzone"]:AddBoxZone("qb-wheelfitment:zone1", vector3(124.47, -3034.8, 6.59), 5.4, 5.0, {
        heading = 269,
        minZ = 5.64,
        maxZ = 7.64,
    ...
    --bennys
    exports["qb-polyzone"]:AddBoxZone("qb-wheelfitment:zone2", vector3(-32.29, -1066.48, 27.94), 4.0, 8.6, {
        name="2",
        heading = 0,
        minZ = 26.5,
        maxZ = 28.1,
...
AddEventHandler("qb-polyzone:enter", function(zone, data)
    ...
    CreateThread(function()
        ...
        QBCore.Functions.TriggerCallback('qb-wheelfitment_sv:getIsWheelFitmentInUse', function(data)
            ...
            exports["qb-ui"]:showInteraction("[E] Wheel Fitment")
            while inZone do
                if IsPedInAnyVehicle(plyPed, false) and not isWheelFitmentInUse then
                    if not isMenuShowing then
                        if PlayerJob.name == "mechanic_tunner" or PlayerJob.name == "mechanic_bennys" or PlayerJob.name == "mechanic_hayes" or PlayerJob.name == "mechanic_gabz" then
