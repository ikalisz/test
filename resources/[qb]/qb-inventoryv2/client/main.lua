...
Citizen.CreateThread(function()
    RegisterKeyMapping('+inv', 'Inventory', 'keyboard', 'F2')
    RegisterKeyMapping('+inv1', 'Inventory [1]', 'keyboard', '1')
    RegisterKeyMapping('+inv2', 'Inventory [2]', 'keyboard', '2')
    RegisterKeyMapping('+inv3', 'Inventory [3]', 'keyboard', '3')
    RegisterKeyMapping('+inv4', 'Inventory [4]', 'keyboard', '4')
    RegisterKeyMapping('+inv5', 'Inventory [5]', 'keyboard', '5')
end)
                    ...
                    elseif CurrentGlovebox ~= nil then
                        if GetVehicleClass(curVeh) == 18 then
                            ...
                            if driverSeat or passengerSeat then
                                ...
                                QBCore.Functions.Notify("You can't open the glove box from the backseat!", "error")
...
RegisterCommand('+generalTab', function()
    ...
    TriggerEvent("tgiannHudMoney", true)
 end, false)
 ...
 function useSlot(slot)
     if not HasNuiFocus and not IsEntityPlayingAnim(PlayerPedId(), "re@construction", "out_of_breath", 1) then
         QBCore.Functions.GetPlayerData(function(PlayerData)
             if not isDead and not PlayerData.metadata["kelepce"] and not PlayerData.metadata["pkelepce"] and not exports["high_phone"]:phoneIsOpen() and not exports["progressbarcircle"]:progressActive() and not exports["progressbardefault"]:progressActive() then
...
AddEventHandler("inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    QBCore.Functions.Progressbar("env", "Item is Produced..", (math.random(10000, 20000) * amount), false, true, {
        ...
        QBCore.Functions.Notify("Unsuccessful!", "error")
        isCrafting = false
	end)
end)
	...
	for k, item in pairs(table) do
		...
		for itemName, itemAmount in pairs(item.costs) do
            if QBCore.Shared.Items[itemName] ~= nil then 
			    if first then
                    first = false 
                    itemCostLabel = "What's needed: " ..itemAmount.. "x " ..QBCore.Shared.Items[itemName]["label"] ..""
                else
...
function kullanKanka(data)
    local crafting = {label = data.label or "Production Desk"}
    crafting.items = {}
    if data.ek then
...
AddEventHandler("inventory:client:PickupSnowballs", function()
    ...
    if not IsPedInAnyVehicle(PlayerPedId) then
        ...
        QBCore.Functions.Progressbar("pickupsnowball", "Gathering..", 1500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
        ...
        QBCore.Functions.Notify("You Cannot Use This Command In The Vehicle!", "error")
    end
end)
...
AddEventHandler("inventory:client:UseWeapon", function(weaponData, time)
    ...
    if gunrpblacklist == true then QBCore.Functions.Notify("You don't know how to use a gun.", "error", 5000) return end
    ...
    if durubality > 0 then
        ...
        if hash ~= -1569615261 then
            ...
            elseif weaponName == "weapon_rpg" then
                QBCore.Functions.Notify("Thanks to our ancestors, we can't take this in our hands anymore", "error", 15000)
        ...
        elseif weaponName == "weapon_stickybomb" then
            QBCore.Functions.Notify("Thanks to our ancestors, we can't take this in our hands anymore", "error", 15000)
        elseif weaponName == "weapon_molotov" then
            QBCore.Functions.Notify("You have to go to the Middle East to throw this.", "error", 15000)
        ...
        QBCore.Functions.Notify("This Weapon Is In Too Bad Condition To Be Used", "error")
    end
        -- end
    -- end) 
end)
...
RegisterCommand('envanterkapat', function()
    exports["elix1rBase"]:SendAlert("Inventory Closing", "info", 3000)
    Wait(2500)
    SetCustomNuiFocus(false, false)
    SendNUIMessage({
        action = "close"
    })
    exports["elix1rBase"]:SendAlert("Inventory Closed", "success")
end)
...
RegisterNUICallback("GiveItem1", function(data, cb)
    ...
    for sayi, esc in pairs(closestPlayer) do
        if GetPlayerServerId(esc) == data.player then
            ...
            TriggerServerEvent('3dme:display', "" ..data.amount.. " Piece " ..data.item.label.. " prolongs", QBCore.Key, "me", "self")
            break
        else
            QBCore.Functions.Notify("There is no one near!", "error", 2500)
        end
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if exports["progressbarcircle"]:progressActive() and exports["progressbardefault"]:progressActive() and  inInventory then
            SendNUIMessage({
                action = "close",
            })
            QBCore.Functions.Notify("You cannot open inventory while trading.", "error")
        end
        Wait(300)
    end
end)
