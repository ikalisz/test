...
function NewDeliveryShift()	
	if MaxDelivery >= 0 then		
		...
		QBCore.Functions.Notify("Proceed to the next delivery point.")
	else
		onJob = false				
		RemoveJobBlip()
		SetNewWaypoint(-425.44, -2787.76, 6.0)
		QBCore.Functions.Notify("You have completed the deliveries, come back.")
	end
end
...
function SpawnGoPostal(x,y,z,h)
	...
	exports['np-fuel']:SetFuel(goPostalVehicle, 100.0)
	local plate = GetVehicleNumberPlateText(goPostalVehicle)	
	TriggerEvent("x-hotwire:give-keys", goPostalVehicle)
end
...
function getParkingPosition(spots)
  ...
  QBCore.Functions.Notify("Parking spaces are completely full, please wait")		  
end
	...
	exports['qb-target']:AddTargetModel('s_m_m_postal_01', {
		options = {
			...
			{ 
				...
				label = "Take your payment and finish the job",
				canInteract = function(entity, data)
		...
		if lisans then 
			if not DoesEntityExist(goPostalVehicle) then 
				if QBCore.Functions.GetPlayerData().money.cash >= 1000 then
					...
					QBCore.Functions.Notify("You don't have enough money for deposit")
				end	
			end
		else
			QBCore.Functions.Notify("You can't get a job because you don't have a driver's license!")
		end
	else
		QBCore.Functions.Notify("You are already working in a job, finish it first.")
	end
end)
	...
	if isDeliverySignedIn then	
		if not onJob then
			if DoesEntityExist(goPostalVehicle) then 																												
				...
				isDeliverySignedIn = false	
				onJob = false	
				QBCore.Functions.Notify("You won't get payment because you lost your delivery tool.")																					
			end	
		else
			QBCore.Functions.Notify("You need to complete all the jobs to get payment.")
		end
	else
		QBCore.Functions.Notify("You need to do business to get payment.")	
	end
end)
