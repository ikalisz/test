                ...
                if weapon ~= -1569615261 and weapon ~= `WEAPON_SNOWBALL` then
                    ...
                    if (CurrentWeaponData and CurrentWeaponData.name == hastToName) then
                        ...
                        if CurrentWeaponData and CurrentWeaponData.name then
                            TriggerEvent("tgiann-hackkoruma:client:kick", "Weapon Use without Serial Number data:" .. CurrentWeaponData.name .." uses:" ..hastToName)
                        else
                            TriggerEvent("tgiann-hackkoruma:client:kick", "Weapon Use without Serial Number data: NULL uses:" ..hastToName)
                        end
                    end
                end
            end
        ...
        if not busy then
            ...
            if QBCore.Shared.Items[hastToName] ~= nil and QBCore.Shared.Items[hastToName]["ammotype"] == type:upper() then
                ...
                if total < maxAmmo then
                    ...
                    QBCore.Functions.Progressbar("ammo_load", "Loading Bullets", 2500, false, true, { -- p1: menu name, p2: yazı, p3: ölü iken kullan, p4:iptal edilebilir
                    ...
                    QBCore.Functions.Notify("The Gun Is Already Loaded", "error") 
                end
            end
        end
    else
        TriggerEvent("tgiann-hackkoruma:client:kick", "Unauthorized Loading Event, Key Sent: "..key)
    end
end)
...
AddEventHandler('weapons:client:SetCurrentWeapon', function(data, bool)
    if data then
        ...
        QBCore.Functions.Notify("You Got The Gun; ".. CurrentWeaponData.label, "success", 1500)
    else
        QBCore.Functions.Notify("You Dropped the Gun; ".. CurrentWeaponData.label, "error", 1500)
...
AddEventHandler('weapons:client:KapaLaWep', function(data, bool)
    if data then
        ...
        QBCore.Functions.Notify("You Dropped Your Gun Because You Got In The Vehicle; ".. CurrentWeaponData.label, "error", 1500)
        CurrentWeaponData = nil
    end
    CanShoot = bool
end)
    ...
    if weapon ~= `WEAPON_UNARMED` then
        ...
        if Config.WeaponAttachments[WeaponData.name] ~= nil then
            if Config.WeaponAttachments[WeaponData.name][attachment] ~= nil then
                ...
                QBCore.Functions.Notify("You Cannot Attach This Attachment to This Weapon!", "error")
            end
        end
    else
        QBCore.Functions.Notify("You Don't Have A Weapon That Can Equip This Attachment!", "error")
    end
end)
