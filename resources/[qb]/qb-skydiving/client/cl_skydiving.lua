...
local waitingTakeOff = 0 -- Elleme la buraya
...
CreateThread(function()
    ...
    AddTextComponentString("Sky Diving")  --haritada gözüken blip adı
...
local function startTimer(Time) 
    ...
    CreateThread(function()
		QBCore.Functions.Notify("The plane is preparing to get up...", "primary")
        while waitingTakeOff do
            if GetGameTimer() < gameTimer + tonumber(1000 * Time) then
                local secondsLeft = GetGameTimer() - gameTimer
                -- drawTxt("Kalkışa Kalan Süre : " .. math.ceil(Time - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
				-- while waitingTakeOff do Wait(0)
				 Wait(100)
				 QBCore.Functions.Notify("Kalan Süre : " .. math.ceil(Time - secondsLeft / 1000), "primary", 1000)
				-- end
            end
			Citizen.Wait(1000)
        end
...
-- RegisterCommand("süre", function()
-- 	local time = 120
-- 	-- local spawnedVehicle = nil
-- 	Citizen.CreateThread(function()
-- 		QBCore.Functions.Notify(time.." Saniyelik Test Sürüşü Başladı!")
-- 		-- TriggerServerEvent("esx_vehicleshop:testPrice")
-- 		while true do
-- 			Citizen.Wait(1000)
-- 			time = time - 1
...
-- RegisterCommand("süre", function(Time)
--     local gameTimer = GetGameTimer()
--     CreateThread(function()
--         while waitingTakeOff do
--             if GetGameTimer() < gameTimer + tonumber(1000 * Time) then
...
--                 drawTxt("Kalkışa Kalan Süre : " .. math.ceil(Time - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
-- 				-- while waitingTakeOff do Wait(0)
-- 				 QBCore.Functions.Notify("Kalkışa Kalan Süre :" .. math.ceil(Time - secondsLeft / 1000), "primary", 1000)
-- 				-- end
--             end
-- 			Citizen.Wait(1000)
--         end
...
exports['qb-target']:AddTargetModel("ig_pilot", {
    options = {
        {
			type = "server",
			event = "qb-skydiving:server:payforgroup",
			icon = "fas fa-car",
			label = "Group Air Diving ($750)",
		},
		{
			type = "client",
			event = "qb-skydiving:client:ridewithbuddy",
			icon = "fas fa-car",
			label = "Jump with a person",
		},
		{
			type = "server",
			event = "qb-skydiving:server:solojump",
			icon = "fas fa-car",
			label = "One man jump",
		}
		-- {
...
RegisterNetEvent('qb-skydiving:client:skydivetime')
AddEventHandler('qb-skydiving:client:skydivetime', function()
	...
	if HasModelLoaded(planeHK) and HasModelLoaded(pilotHK) then
		...
		exports["np-fuel"]:SetFuel(Skydive, 100.0)
		SetEntityAsMissionEntity(Skydive, true, true)
		SetModelAsNoLongerNeeded(Skydive)
		TriggerEvent("x-hotwire:give-keys", GetVehicleNumberPlateText(vehicle))
		...
		TriggerServerEvent('qb-skydiving:flightcooldown') 
...
RegisterNetEvent('qb-skydiving:client:skydivesolo')
AddEventHandler('qb-skydiving:client:skydivesolo', function()
	local Ped = PlayerPedId()
	DoScreenFadeOut(2000)
	Wait(4000)
...
RegisterNetEvent('qb-skydiving:client:ridewithbuddy')
AddEventHandler('qb-skydiving:client:ridewithbuddy', function()
