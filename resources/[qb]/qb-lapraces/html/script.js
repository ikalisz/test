    ...
    if (type == "creator") {
        if (data.active) {
            if (!CreatorActive) {
                ...
                $("#editor-keys-tiredistance").html('<span style="color: rgb(0, 201, 0);">+ INSERT </span> / <span style="color: rgb(255, 43, 43);">- [</span> - ['+data.data.TireDistance+'.0]');
        ...
        if (data.active) {
            if (!RaceActive) {
                ...
                $("#race-checkpoints").html('Checkpoints: ' + data.data.CurrentCheckpoint + ' / ' + data.data.TotalCheckpoints);
                if (data.data.TotalLaps == 0) {
                    $("#race-lap").html('Round: Single Round');
                } else {
                    $("#race-lap").html('Round: ' + data.data.CurrentLap + ' / ' + data.data.TotalLaps);
                }
                $("#race-time").html('Lap Duration: ' + secondsTimeSpanToHMS(data.data.Time));
                if (data.data.BestLap !== 0) {
                    $("#race-besttime").html('Best Tour: ' + secondsTimeSpanToHMS(data.data.BestLap));
                } else {
                    $("#race-besttime").html('Best Tour: N/A');
                }
                $("#race-totaltime").html('Toplam Süre: ' + secondsTimeSpanToHMS(data.data.TotalTime));
            } else {
                $("#race-racename").html('Race: ' + data.data.RaceName);
                $("#race-checkpoints").html('Checkpoints: ' + data.data.CurrentCheckpoint + ' / ' + data.data.TotalCheckpoints);
                if (data.data.TotalLaps == 0) {
                ...
                $("#race-totaltime").html('The total time: ' + secondsTimeSpanToHMS(data.data.TotalTime));
            }
        } else {
            RaceActive = false;
            $(".editor").hide();
