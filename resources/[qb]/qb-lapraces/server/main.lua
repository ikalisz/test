...
RegisterNetEvent('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, 'Youre the only one left in the race. Thats why the race is over.', 'error')
...
RegisterNetEvent('qb-lapraces:server:SaveRace', function(RaceData)
    ...
    exports.oxmysql:execute('INSERT INTO lapraces (name, checkpoints, creator, distance, raceid) VALUES (?, ?, ?, ?, ?)',
...
QBCore.Functions.CreateCallback('qb-lapraces:server:GetTrackData', function(source, cb, RaceId)
    local result = exports.oxmysql:execute('SELECT * FROM players WHERE citizenid = ?', {Races[RaceId].Creator})
...
QBCore.Commands.Add("yarışiptal", "Cancel going race..", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
...
CreateThread(function()
    local races = exports.oxmysql:execute('SELECT * FROM lapraces', {})
    if races[1] ~= nil then
        for k, v in pairs(races) do
            local Records = {}
