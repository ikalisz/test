        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        QBCore.Functions.Notify('No more checkpoint...', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        QBCore.Functions.Notify('To be at least 10 checkpoints', 'error')
                    end
                end

                if IsControlJustPressed(0, 121) or IsDisabledControlJustPressed(0, 121) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        CreatorData.TireDistance = CreatorData.TireDistance + 1.0
                    else
                        QBCore.Functions.Notify('You cant do bigger')
                    end
                end

                if IsControlJustPressed(0, 178) or IsDisabledControlJustPressed(0, 178) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        CreatorData.TireDistance = CreatorData.TireDistance - 1.0
                    else
                        QBCore.Functions.Notify('You cant make it smaller.')
                    end
                end
            else
                local coords = GetEntityCoords(PlayerPedId())
                DrawText3Ds(coords.x, coords.y, coords.z, 'You must be in the vehicle')
            end
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    QBCore.Functions.Notify('To confirm again [9) press.', 'error', 5000)
                else
                    for id, CheckpointData in pairs(CreatorData.Checkpoints) do
                        if CheckpointData.blip ~= nil then
                            RemoveBlip(CheckpointData.blip)
                    ...
                    CreatorData.RaceName = nil
                    CreatorData.Checkpoints = {}
                    QBCore.Functions.Notify('Editor was canceled!', 'error')
...
local function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        QBCore.Functions.Notify('You finished the race in this time: '..SecondsToClock(CurrentRaceData.TotalTime)..', Best tour: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        QBCore.Functions.Notify('You finished the race in this time: '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('qb-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        QBCore.Functions.Notify('You edit a race already.', 'error')
    end
end)
...
RegisterNetEvent('qb-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        ...
        QBCore.Functions.Notify('You are already in a race..', 'error')
    end
end)

RegisterNetEvent('qb-lapraces:client:LeaveRace', function(data)
    QBCore.Functions.Notify('You finished the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
            CurrentRaceData.Checkpoints[k].blip = nil
...
RegisterNetEvent('qb-lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    QBCore.Functions.Notify('The race will begin in 10 seconds.', 'error', 2500)
        ...
        if CurrentRaceData.RaceName ~= nil then
            ...
            QBCore.Functions.Notify('Start!', 'success', 1000)
        ...
        QBCore.Functions.Notify('You are not in a race now..', 'error')
    end
end)
...
RegisterNetEvent('qb-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            QBCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' The person finished the race. Sequence: '..Place, 'error', 7500)
        end
    end
end)
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            QBCore.Functions.Notify('If you dont come back, you will be expelled from the race: '..ToFarCountdown..'s', 'error', 500)
                        else
    ...
    while true do
        if RaceData.InCreator then
            ...
            if PlayerVeh ~= 0 then
                ...
                DrawText3Ds(Offset.left.x, Offset.left.y, Offset.left.z, 'Checkpoint Left')
                DrawText3Ds(Offset.right.x, Offset.right.y, Offset.right.z, 'Checkpoint Right')
            end
        end
        Wait(3)
    end
