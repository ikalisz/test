...
function IsPlayerAdmin() 
    ...
    QBCore.Functions.TriggerCallback('mc-adminmenu/server/get-permission', function(IsAdmin)
...
function RefreshMenu(Type)
    ...
    elseif Type == 'Settings'then
        if GetResourceKvpString("mc-adminmenu-settings") == nil or GetResourceKvpString("mc-adminmenu-settings") == "[]" then
            Config.AdminSettings = GenerateAdminSettings()
            SetResourceKvp("mc-adminmenu-settings", json.encode(Config.AdminSettings))
        else
            Config.AdminSettings = json.decode(GetResourceKvpString("mc-adminmenu-settings"))
        end
...
function GetPlayersInArea(Coords, Radius)
	...
	QBCore.Functions.TriggerCallback('mc-admin/server/get-active-players-in-radius', function(Players)
...
function GetPlayers()
    local Promise = promise:new()
    QBCore.Functions.TriggerCallback("mc-admin/server/get-players", function(Players)
