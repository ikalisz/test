...
-- RegisterCommand('adminmenu', function(source, args, RawCommand) TriggerEvent('mc-admin/client/try-open-menu', true) end, false)

-- [ Events ] --

RegisterNetEvent('mc-admin/client/try-open-menu', function(OnPress)
...
RegisterNUICallback('Admin/ToggleSetting', function(Data, Cb)
    ...
    SetKvp("mc-adminmenu-settings", json.encode(Config.AdminSettings), "Settings")
    Cb('Ok')
end)

RegisterNUICallback('Admin/GetCharData', function(Data, Cb)
    QBCore.Functions.TriggerCallback('mc-admin/server/get-player-data', function(PlayerData)
        Cb(PlayerData)
    end, Data.License)
end)
