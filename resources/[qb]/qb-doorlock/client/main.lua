...
-- {"boss":false,"gradelabel":"Officer II","onduty":true,"paySafe":false,"label":"Polis","name":"police","payment":175,"grade":3}
local function isAuthorized(door)
	if door.allAuthorized then return true end

	if door.authorizedJobs then
		if door.authorizedJobs[PlayerData.job.name] and PlayerData.job.grade >= door.authorizedJobs[PlayerData.job.name] then
			return true
	...
	-- 	if door.authorizedGangs[PlayerData.gang.name] and PlayerData.gang.grade >= door.authorizedGangs[PlayerData.gang.name] then
	-- 		return true
...
RegisterCommand("doorlockloaded", function()
	QBCore.Functions.TriggerCallback('qb-doorlock:server:setupDoors', function(result)
		Config.DoorList = result
		PlayerData = QBCore.Functions.GetPlayerData()
		isLoggedIn = true
				...
				while isLoggedIn do
					...
					elseif paused then
						...
						if closestDoor.distance < (closestDoor.data.distance or closestDoor.data.maxDistance) then
							...
							if displayText ~= "" then exports['qb-ui']:showInteraction(displayText) end
						else
