...
local function loadSettings(settings)
    for k,v in pairs(settings) do
        ...
        elseif k == 'isToggleProgbarShapeChecked' then
            Menu.isToggleProgbarShapeChecked = v
            SendNUIMessage({ test = true, event = k, toggle = v})
            TriggerEvent("progressbar:sekildegistir", v)
        elseif k == 'isToggleNotifyShapeChecked' then
            Menu.isToggleNotifyShapeChecked = v
            SendNUIMessage({ test = true, event = k, toggle = v})
            TriggerEvent("notify:sekildegistir", v)
...
-- RegisterKeyMapping('menu', 'Open Menu', 'keyboard', "I")
...
RegisterNUICallback('ToggleProgbarShape', function(_, cb)
    Wait(50)
        Menu.isToggleProgbarShapeChecked = Menu.isToggleProgbarShapeChecked == "yuvarlak" and "kare" or "yuvarlak"
        Wait(50)
        TriggerEvent("progressbar:sekildegistir", Menu.isToggleProgbarShapeChecked)
    saveSettings()
    cb("ok")
end)
RegisterNUICallback('ToggleNotifyShape', function(_, cb)
    Wait(50)
        Menu.isToggleNotifyShapeChecked = Menu.isToggleNotifyShapeChecked == "ikonlu" and "ikonsuz" or "ikonlu"
        Wait(50)
        TriggerEvent("notify:sekildegistir", Menu.isToggleNotifyShapeChecked)
    saveSettings()
    cb("ok")
end)
...
local hudgidersenananisikerim = false
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in qb-core
    hunger = newHunger
    thirst = newThirst
    if not hudgidersenananisikerim then
        hudgidersenananisikerim = true
    end
end)
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['np-fuel']:GetFuel(vehicle))
    end
...
CreateThread(function()
    ...
    while true do
        Wait(100)
        if hudgidersenananisikerim then
            ...
            if isLoggedIn then
                ...
                playerDead = IsEntityDead(player) or QBCore.Functions.GetPlayerData().metadata['isdead'] or false
    ...
    while true do
        if hudgidersenananisikerim then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsThisModelABicycle(GetEntityModel(GetVehiclePedIsIn(ped, false))) then
                if exports["np-fuel"]:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        TriggerServerEvent("InteractSound_SV:PlayOnSource", "pager", 0.2)
    ...
    while true do
        ...
        if isLoggedIn then
            ...
            if hudgidersenananisikerim then
                ...
                TriggerServerEvent("QBCore:Server:SetMetaData", "hunger", QBCore.Functions.GetPlayerData().metadata["hunger"])
                TriggerServerEvent("QBCore:Server:SetMetaData", "thirst", QBCore.Functions.GetPlayerData().metadata["thirst"])
                while not armorcugeldi do
                    time = 15000
                    local armoramk = QBCore.Functions.GetPlayerData().metadata["armor"]
                    SetPedArmour(PlayerPedId(), armoramk)
                    armorcugeldi = true
                end
                TriggerServerEvent("QBCore:Server:SetMetaData", "armor", GetPedArmour(PlayerPedId()))
            end
        end
        Wait(time)
    end
...
RegisterNetEvent('haso:load-data')
AddEventHandler('haso:load-data', function(health, armour)
...
CreateThread(function()
    while true do
        ...
        local araba = GetVehiclePedIsIn(PlayerPedId(), false)
        if IsEntityInAir(araba) then
...
RegisterCommand('soygunsifirla', function()
    TriggerServerEvent('elixir-base:soygunKontrolReset')
end)
    ...
    while true do
        ...
        if hudgidersenananisikerim then
            if QBCore.Functions.GetPlayerData().job.onduty then
                TriggerServerEvent("QBCore:Server:SetMetaData", "dutytime", QBCore.Functions.GetPlayerData().metadata["dutytime"] + 1)
            end
        end
    end
end)
...
AddEventHandler('seatbelt:client:UseHarness', function(ItemData)
    ...
    if inveh and class ~= 8 and class ~= 13 and class ~= 14 then
        if not harnessOn then
            QBCore.Functions.Progressbar("harness_equip", "Racing Belt Attaching..", 2000, false, false, {
            ...
            QBCore.Functions.Progressbar("harness_equip", "Racing Belt Removing..", 2000, false, false, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
...
function ToggleSeatbelt()
    if seatbeltOn then
        ...
        TriggerEvent('QBCore:Notify', "Belt removed!", "error")
    else
        seatbeltOn = true
        TriggerEvent("seatbelt:client:ToggleSeatbelt")
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "tak", 0.5)
        TriggerEvent('QBCore:Notify', "Belt fastened!")
    end
end
...
CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(PlayerPedId()) and GetPedInVehicleSeat(currentVehicle, -1) and seatbeltOn == false and vehicleClass ~= 13 and vehicleClass ~= 8 and vehicleClass ~= 21 and vehicleClass ~= 14 and vehicleClass ~= 16 and vehicleClass ~= 15 then
            TriggerEvent('InteractSound_CL:PlayOnOne', 'alarm', 0.3)
        end
        Wait(10000)
    end
end)
...
function ToggleHarness()
    if harnessOn then
        ...
        TriggerEvent('QBCore:Notify', "Race belt removed!", "error")
    else
        harnessOn = true
        TriggerEvent("seatbelt:client:ToggleHarness")
        TriggerEvent('QBCore:Notify', "Race belt fastened!")
    end
end
