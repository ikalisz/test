...
Config.FrameworkFunctions.registerCallback("high_phone:getBankData", function(_ARG_0_, cb)
  ...
  local r = MySQL.Sync.prepare("SELECT charinfo FROM `players` WHERE `citizenid` = ?", {Config.FrameworkFunctions.getPlayer(src).identifier})
  local Player = QBCore.Functions.GetPlayer(src)
    data.id = 1
    data.holder = GetCharName(src)
    data.number = "1234 5678 9123 4567"
    -- data.statement, data.cards, data[1], data.date = {}, {}, {}, "01/33"
    data.iban = json.decode(r).iban
...
RegisterServerEvent("high_phone:sendBillVale", function(amount)
  ...
  if Player then
    ...
    if amount > 0 then
      MySQL.Async.insert('INSERT INTO billing (citizenid, amount, society, sender, sendercitizenid) VALUES (@citizenid, @amount, @society, @sender, @sendercitizenid)', {
    ...
    TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, 'Yeni Faturan Var')
    end
  end
end)
...
Config.FrameworkFunctions.registerCallback('high_phone:payInvoice', function(source, cb, id)
  local invoices = MySQL.Sync.fetchAll('SELECT * FROM billing WHERE id = @id', {["@id"] = id})

  if(invoices[1] ~= nil) then
      local Ply = QBCore.Functions.GetPlayer(source)
      local SenderPly = QBCore.Functions.GetPlayerByCitizenId(invoices[1].sendercitizenid)
      if Ply.Functions.RemoveMoney('bank', invoices[1].amount, "paid-invoice") then 
        ...
        TriggerClientEvent("high_phone:sendNotification", Ply.PlayerData.source, "Bank", " " .. invoices[1].amount .. "$ Tutarında ki fatura ödendi!", 3000)
        QBCore.Functions.addJobMoney(invoices[1].society, invoices[1].amount)
        MySQL.Sync.execute('DELETE FROM billing WHERE id = @id', {["@id"] = id})
        TriggerClientEvent('high_phone:updateBankData', Ply.PlayerData.source) 
        cb("SUCCESS")
      else
        TriggerClientEvent("high_phone:sendNotification", Ply.PlayerData.source, "Bank", "Faturayı ödeyebilecek kadar paran yok!", 3000)
      end
    end
end)
