        ...
        for k, v in pairs(vehicles) do
            ...
            for key,value in pairs(QBCore.Shared.Vehicles) do
              for kkey,vvalue in pairs(value) do
                if vvalue.model == v.carName then 
                  dosya = vvalue
                  break
                end
              end
              if dosya then break end
            end
            -- if dosya then 
                data[#data + 1] = {
                    brand = "",
                    name = dosya and dosya.name or v.carName,
                    plate = v.plate,
                    garage = (v.garaj and string.len(v.garaj) > 0) and (not v.stored and "vehicle outside" or v.garaj) or "Ground"
    ...
    if not AracCikariliyor then
        ...
        for i = 1, #gameVehicles do
            if DoesEntityExist(gameVehicles[i]) then
                if GetVehicleNumberPlateText(gameVehicles[i]) == plate then
                    ...
                    QBCore.Functions.Notify(plate .. " plated vehicle outside or towed.", "error")
                    iptal()
                    return
                end
            end
        ...
        QBCore.Functions.TriggerCallback('garage:fetchVehicleProperties', function(properties)
            if properties == nil then
                QBCore.Functions.Notify("The car is not in the garage.", "error", 4500)
                return
            end
            wait = 1000 * math.random(15, 20)
            QBCore.Functions.Notify("Your vehicle is brought by the valet. Estimated waiting time: " .. wait / 1000,
            "success")
            Wait(wait)
            QBCore.Functions.SpawnVehicle(properties.model, function(veh)
                QBCore.Functions.SetVehicleProperties(veh, properties)
                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(PlayerId()), 'vale', "Valet Fee", Config.ValePrice)
                TriggerClientEvent("x-hotwire:give-keys", QBCore.Functions.GetPlate(veh))
                QBCore.Functions.Notify("Vehicle removed.", "success", 4500)
                cb("ok")
            end, spawnPoint, true)
        end, plate)
        iptal()
...
local Vehicles = {
    {label = 'Police Victoria', spawnName = 'polvic'},
    {label = 'Mazda FN RX7', spawnName = 'fnfrx7'},
    {label = 'Subaru WRX', spawnName = 'subwrx'},
    {label = 'PD Ford Victoria', spawnName = 'npolvic'}
}
