...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3Queue System: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals,point,roleStrings)
    ...
    if Config.enableDiscordWhitelist then
        ...
        for pos, data in ipairs(queueList) do
            if tmp.priority then
                ...
                if _pos then
                    Queue:DebugPrint(string_format("%s[%s] priority given and queued %d/%d", tmp.name, ids[1], _pos, queueCount))
                    break
                end
            end
        end
    ...
    if not _pos then
        ...
        Queue:DebugPrint(string_format("%s[%s] %d/%d Queued up", tmp.name, ids[1], _pos, queueCount))
    end
...
local ra1derkart = {
    ...
    body = { 
        {
            type = "Image",
            url = "https://media.discordapp.net/attachments/1071168371112018123/1112475910596210721/Untitled-1.png",
            horizontalAlignment = "Center",
            width = "400px",
            height = "400px"
        }, 
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local function GeriDonus(data,rawData)
        if data.secenek == "giris" then
            local src = oyuncu
    ...
    deferrals.presentCard(ra1derkart,GeriDonus)
end
