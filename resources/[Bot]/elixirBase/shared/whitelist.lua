...
function loadWhiteList(cb)
	TgiWhiteList = {}
    blackList = {}
	exports.oxmysql:execute('SELECT * FROM whitelist', {   
    }, function(identifiers)
        for i=1, #identifiers, 1 do
            TgiWhiteList[identifiers[i].identifier] = identifiers[i].discord
		end
	end)

    exports.oxmysql:execute('SELECT * FROM blackList', {   
    }, function(data)
        blackList = data
	end)
end

RegisterCommand("inaktif", function(source)
    if source == 0 then
        local no = 0
        local dcBot = 0
        local text = ""
        exports.oxmysql:execute('SELECT * FROM whitelist', {   
            ...
            local text = ""
            for i=1, #identifiers, 1 do
                if identifiers[i].songiris == nil then identifiers[i].songiris = "01-01-2000" end
                no = no + 1
                dcBot = dcBot + 1
                local timeToConvert = identifiers[i].songiris
                local runday, runmonth, runyear = timeToConvert:match("(%d+)-(%d+)-(%d+)")
                local convertedTimestamp = os.time({year = runyear, month = runmonth, day = runday})
                
                text = text .. " No:"..no.. " " .. identifiers[i].identifier .. " " .. identifiers[i].songiris .. " <@!" .. identifiers[i].discord .."> İnaktif Süresi: ".. QBCore.Shared.Round((os.time() - convertedTimestamp) / 86400) .." \n"
                if dcBot == 5 then
                    dcBot = 0
                    PerformHttpRequest("", function() end, 'POST', json.encode({username = Config.SystemName, content = text, avatar_url = Config.SystemAvatar, tts = false}), {['Content-Type'] = 'application/json'})
                    text = ""
                end
                Citizen.Wait(500)
            end
...
RegisterCommand("wlynl", function(source)
	if source == 0 then
		loadWhiteList()
	end
end)
...
local discordRoleControl = true
RegisterCommand("discord", function(source)
    if source == 0 then
        discordRoleControl = not discordRoleControl
        print("Discord Role Control: ".. tostring(discordRoleControl))
    end
end)
