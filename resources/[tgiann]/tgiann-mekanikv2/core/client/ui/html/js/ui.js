window.onload = function(e)
{
    ...
    function toggleMenu(state, menu)
    {
        if(state)
        {
            if(menuStructure[menu].selectedItem < maxMenuItems)
            {
                ...
                $.post("http://tgiann-mekanikv2/selectedItem", JSON.stringify({
                    id: val1,
                    item: val2,
                    item2: val3
                }));
    ...
    function updateItem2TextOnly(menu, id, text)
    {
        ...
        $.post("http://tgiann-mekanikv2/updateItem2", JSON.stringify({
            item: text
        }));
    }
    ...
    function updateItem2Text(menu, id, text)
    {
        ...
        else if(id != menuStructure[menu].previousSelectedItemID)
        {
            ...
            let arry = menuStructure[menu].itemsArray[prevID+1]
            if (!arry) {
                arry =  menuStructure[menu].itemsArray[id]
            }
            arry.getElementsByClassName("item2")[0].textContent = menuStructure[menu].items[prevID].item2;
            menuStructure[menu].previousSelectedItemID = id;
